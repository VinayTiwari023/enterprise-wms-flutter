        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              Text("Purchase Order $poNumber not found", 
          ...
          CustomScrollView(
            ...
            slivers: [
              SliverAppBar(
                ...
                actions: [
                  TextButton.icon(
                    onPressed: () => ref.read(inboundViewModelProvider.notifier).receiveByLPN(poNumber, "LPN-AUTO"),
                    icon: const Icon(Icons.inventory_2_outlined, size: 18),
                    label: const Text("Receive LPN", style: TextStyle(fontSize: 12)),
                  ),
                ],
              ),
              SliverPadding(
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Text("${po.itemsList.length} SKUs", style: const TextStyle(color: Colors.grey, fontSize: 13)),
                        ],
                      ),
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (item.damagedQty > 0)
                                    Text("Reason: ${item.damageReason}", style: const TextStyle(color: Colors.redAccent, fontSize: 11)),
                                ],
                              ),
                            ),
                            Row(
                              children: [
                                IconButton(
                                  icon: const Icon(Icons.report_problem_outlined, size: 20, color: Colors.orange),
                                  onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => DamageReportView(poNumber: poNumber, sku: item.sku, itemName: item.name))),
                                ),
                                GestureDetector(
                                  onTap: () => ref.read(inboundViewModelProvider.notifier).updateItemQuantity(poNumber, item.sku, received: 1),
                                  child: Container(
              ...
              child: StartReceivingButton(
                ...
                onTap: () {
                  Navigator.pushReplacement(
                    context, 
                    MaterialPageRoute(builder: (_) => GRNSummaryView(poNumber: poNumber))
                  );
                },
                text: po.status == "Pending" ? "Start Receiving" : 
                      po.status == "Partial" ? "Resume Receiving" : "Review GRN",
              ),
            ),
          ),
        ],
